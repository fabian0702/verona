...
func (ws *Websocket) wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case "stop_proxy":
			...
			fmt.Printf("Stopping proxy with ID: %s\n", stopProxy.ProxyID)
		...
		case "unpause":
			...
			fmt.Printf("Unpausing proxy with ID: %s, Pause: %t\n", unpauseCommand.ProxyID, unpauseCommand.Pause)
...
func (ws *Websocket) Start() {
	...
	fmt.Printf("Server binding to %s", ws.SocketPath)
	// Remove socket if it already exists
	if _, err := os.Stat(ws.SocketPath); err == nil {
		if err := os.Remove(ws.SocketPath); err != nil {
			fmt.Printf("Error removing existing socket: %v\n", err)
			return
		}
	}
	...
	fmt.Printf("WebSocket server started on Unix socket: %s\n", ws.SocketPath)
